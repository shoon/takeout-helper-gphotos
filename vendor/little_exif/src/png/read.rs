...
pub(super) fn read_chunk_length<T: Seek + Read>(cursor: &mut T) -> Result<u32, std::io::Error> {
...
pub(super) fn read_chunk_name<T: Seek + Read>(cursor: &mut T) -> Result<String, std::io::Error> {
...
pub(super) fn read_chunk_crc<T: Seek + Read>(cursor: &mut T) -> Result<[u8; 4], std::io::Error> {
    let field = read_4_bytes(cursor)?;
    return Ok(field);
}
