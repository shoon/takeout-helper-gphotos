...
fn check_signature(file_buffer: &Vec<u8>) -> Result<Cursor<&Vec<u8>>, std::io::Error> {
...
fn file_check_signature(path: &Path) -> Result<File, std::io::Error> {
...
pub(crate) fn vec_parse_png(file_buffer: &Vec<u8>) -> Result<Vec<PngChunk>, std::io::Error> {
...
/// - Can the file be opened and is the signature valid?
/// - Are the various chunks OK or not? For this, the local subroutine `get_next_chunk_descriptor` is used
pub(crate) fn file_parse_png(path: &Path) -> Result<Vec<PngChunk>, std::io::Error> {
    let mut file = file_check_signature(path)?;
    return generic_parse_png(&mut file);
}

fn generic_parse_png<T: Seek + Read>(cursor: &mut T) -> Result<Vec<PngChunk>, std::io::Error> {
...
fn get_next_chunk_descriptor<T: Seek + Read>(cursor: &mut T) -> Result<PngChunk, std::io::Error> {
    ...
    let png_chunk_result = PngChunk::from_string(&chunk_name.clone(), chunk_length);

    match png_chunk_result {
        Ok(png_chunk) => return Ok(png_chunk),
        Err(e) => {
...
fn generic_read_metadata<T: Seek + Read>(
    cursor: &mut T,
    ...
    for chunk in parsed_png {
        match chunk.as_string().as_str() {
            "eXIf" => {
                ...
                let eXIf_chunk_data = read_chunk_data(cursor, chunk.length() as usize)?;
            ...
            "tEXt" | "zTXt" | "iTXt" => {
                ...
                let decompressed_data = get_data_from_text_chunk(chunk_name.as_str(), &chunk_data)?;
...
fn remove_chunk_at(cursor: &mut Cursor<&mut Vec<u8>>) -> Result<(), std::io::Error> {
    ...
    range_remove(cursor.get_mut(), chunk_start_position, chunk_end_position);
...
fn clear_exif_from_xmp_metadata(
    ...
    let text_chunk_data = get_data_from_text_chunk(chunk_name.as_str(), chunk_data)?;
    let clean_xmp_data = match remove_exif_from_xmp(&text_chunk_data) {
        Ok(data) => data,
        Err(e) => {
            return io_error!(Other, format!("Failed to remove EXIF from XMP data: {}", e));
        }
    };
...
pub(crate) fn file_write_metadata(path: &Path, metadata: &Metadata) -> Result<(), std::io::Error> {
    // First clear the existing metadata
    // This also parses the PNG and checks its validity, so it is safe to
    // assume that is, in fact, a usable PNG file
...
#[allow(non_snake_case)]
fn write_chunk<T: Seek + Read + Write>(
    cursor: &mut T,
    chunk_name: &str,
    chunk_data: &[u8],
) -> Result<(), std::io::Error> {
...
fn generic_write_metadata<T: Seek + Read + Write>(
    ...
    let zTXt_chunk_data: Vec<u8> = construct_zTXt_chunk_data(&Vec::new(), &encoded_metadata);
...
fn encode_metadata_png(exif_vec: &Vec<u8>) -> Vec<u8> {
    ...
    // Write ssss, padded to 8 bytes with leading spaces
    png_exif.extend(vec![SPACE; 8 - ssss.len()]);
...
fn decode_metadata_png(encoded_data: &Vec<u8>) -> Result<Vec<u8>, std::io::Error> {
    ...
    for byte in encoded_data {
        ...
        let value_string =
            "".to_owned() + &(other_byte as char).to_string() + &(*byte as char).to_string();
...
#[allow(non_snake_case)]
pub(crate) fn as_u8_vec(general_encoded_metadata: &Vec<u8>, as_zTXt_chunk: bool) -> Vec<u8> {
    ...
    return construct_zTXt_chunk_data(&[0x7a, 0x54, 0x58, 0x74], &basic_png_encode_result);
}

#[allow(non_snake_case)]
fn construct_zTXt_chunk_data(prefix: &[u8], basic_png_encode_result: &[u8]) -> Vec<u8> {
