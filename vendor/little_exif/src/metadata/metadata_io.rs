...
impl Metadata {
    ...
    pub fn new_from_vec(
        ...
        let raw_pre_decode_general = match file_type {
            FileExtension::HEIF => heif::read_metadata(file_buffer),
            FileExtension::JPEG => jpg::read_metadata(file_buffer),
            FileExtension::JXL => jxl::read_metadata(file_buffer),
            FileExtension::PNG { as_zTXt_chunk: _ } => png::read_metadata(file_buffer),
            FileExtension::TIFF => tiff::vec::read_metadata(file_buffer),
    ...
    pub fn new_from_path(path: &Path) -> Result<Metadata, std::io::Error> {
        ...
        if let Some(extension_based) = extension_based_file_type_opt {
            ...
            if let Some(content_based_file_type) = content_based_file_type {
                ...
                return io_error!(Other, "Could not determine file type when reading file!");
            }
        }
        ...
        if let Some(content_based_file_type) = content_based_file_type {
            ...
            log::warn!(
                "Could not determine file type based on content, fall back on file extension"
            );
        }
        ...
        let raw_pre_decode_general = match file_type {
            FileExtension::HEIF => heif::file_read_metadata(path),
            FileExtension::JPEG => jpg::file_read_metadata(path),
            FileExtension::JXL => jxl::file_read_metadata(path),
            FileExtension::PNG { as_zTXt_chunk: _ } => png::file_read_metadata(path),
            FileExtension::TIFF => tiff::file::read_metadata(path),
    ...
    pub fn clear_metadata(
        ...
        match file_type {
            ...
            FileExtension::PNG { as_zTXt_chunk: _ } => png::clear_metadata(file_buffer),
            FileExtension::TIFF => tiff::vec::clear_metadata(file_buffer),
    ...
    pub fn file_clear_metadata(path: &Path) -> Result<(), std::io::Error> {
        ...
        match file_type {
            FileExtension::HEIF => heif::file_clear_metadata(path),
            FileExtension::JPEG => jpg::file_clear_metadata(path),
            FileExtension::JXL => jxl::file_clear_metadata(path),
            FileExtension::PNG { as_zTXt_chunk: _ } => png::file_clear_metadata(path),
            FileExtension::TIFF => tiff::file::clear_metadata(path),
    ...
    /// for further details
    #[allow(unreachable_patterns)]
    pub fn as_u8_vec(&self, for_file_type: FileExtension) -> Result<Vec<u8>, std::io::Error> {
    ...
    pub fn write_to_vec(
        ...
        match file_type {
            ...
            FileExtension::JXL => jxl::write_metadata(file_buffer, self),
            FileExtension::PNG { as_zTXt_chunk: _ } => png::write_metadata(file_buffer, self),
            FileExtension::TIFF => tiff::vec::write_metadata(file_buffer, self),
            FileExtension::WEBP => webp::vec::write_metadata(file_buffer, self),
            _ => {
                return io_error!(
                    Other,
                    format!(
    ...
    pub fn write_to_file(&self, path: &Path) -> Result<(), std::io::Error> {
        ...
        match file_type {
            FileExtension::HEIF => heif::file_write_metadata(path, self),
            FileExtension::JPEG => jpg::file_write_metadata(path, self),
            FileExtension::JXL => jxl::file_write_metadata(path, self),
            FileExtension::PNG { as_zTXt_chunk: _ } => png::file_write_metadata(path, self),
            FileExtension::TIFF => tiff::file::write_metadata(path, self),
            FileExtension::WEBP => webp::file::write_metadata(path, self),
            _ => {
                return io_error!(
                    Other,
                    format!(
