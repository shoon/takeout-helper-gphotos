...
impl Metadata {
    ...
    pub fn create_ifd(&mut self, ifd_type: ExifTagGroup, generic_ifd_nr: u32) {
    ...
    pub(crate) fn general_decoding_wrapper(
        ...
        if let Ok(pre_decode_general) = raw_pre_decode_general {
            ...
            if let Ok((endian, image_file_directories)) = decoding_result {
                ...
                return io_error!(
                    Other,
                    "Unreachable code reached in Metadata::general_decoding_wrapper (1)"
                );
            }
            ...
            return io_error!(
                Other,
                "Unreachable code reached in Metadata::general_decoding_wrapper (2)"
            );
        }
    }
    ...
    pub fn encode(&self) -> Result<Vec<u8>, std::io::Error> {
        ...
        for ifd in self.image_file_directories.iter() {
            ifds_with_offset_info_only.push(ImageFileDirectory::new_with_tags(
                vec![],
                ifd.get_ifd_type(),
        ...
        for n in 0..=generic_ifd_count {
            ...
            let Some(last_ifd) = filter_result.last() else {
                continue;
            };
    ...
    fn sort_data(&mut self) {
        self.image_file_directories.sort_by(|a, b| {
    ...
    fn decode(
        ...
        if !((endian == Endian::Little && magic_number_buffer == [0x2a, 0x00])
        ...
        let mut ifd_offset_option =
            Some(from_u8_vec_res_macro!(u32, &ifd0_offset_buffer, &endian)?);
...
mod tests {
    ...
    #[test]
    fn new_test_1() -> Result<(), std::io::Error> {
    ...
    #[ignore]
    #[test]
    fn new_test_2() -> Result<(), std::io::Error> {
