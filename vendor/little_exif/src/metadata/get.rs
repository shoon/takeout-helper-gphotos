...
impl Metadata {
    ...
    pub fn get_ifds(&self) -> &Vec<ImageFileDirectory> {
    ...
    pub fn get_ifd(&self, group: ExifTagGroup, generic_ifd_nr: u32) -> Option<&ImageFileDirectory> {
        self.image_file_directories
            .iter()
            .find(|ifd| ifd.get_generic_ifd_nr() == generic_ifd_nr && ifd.get_ifd_type() == group)
    }

    /// Gets the maximum generic ifd number that any of the struct's IFDs has
    pub fn get_max_generic_ifd_number(&self) -> u32 {
        if let Some(max_generic_ifd) = self
    ...
    pub fn get_ifd_mut(
        ...
        if !self
            .image_file_directories
            .iter()
            .any(|ifd| ifd.get_generic_ifd_nr() == generic_ifd_nr && ifd.get_ifd_type() == group)
        {
...
impl Metadata {
    pub fn get_tag(&self, tag: &ExifTag) -> GetTagIterator<'_> {
    ...
    pub fn get_tag_by_hex(&self, hex: u16, group: Option<ExifTagGroup>) -> GetTagIterator<'_> {
        GetTagIterator {
            metadata: self,
            current_ifd_index: 0,
            current_tag_index: 0,
...
impl<'a> Iterator for GetTagIterator<'a> {
    type Item = &'a ExifTag;
