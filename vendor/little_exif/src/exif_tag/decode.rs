...
pub(crate) fn decode_tag_with_format_exceptions(
    ...
    if raw_tag.format().as_u16() != format.as_u16() {
        ...
        match (raw_tag.format(), format) {
            ...
            (ExifTagFormat::INT32U, ExifTagFormat::INT16U) => {
                ...
                let int32u_data = int16u_data
                    ...
                    .map(|x| x as u32)
                    .collect::<Vec<u32>>();
                return raw_tag.set_value_to_int32u_vec(int32u_data).map_err(|e| {
                    io_error_plain!(Other, format!("Could not decode tag {:?}: {}", raw_tag, e))
                });
            }
            ...
            (ExifTagFormat::INT16U, ExifTagFormat::INT32U) => {
                ...
                let int16u_data = int32u_data
                    ...
                    .map(|x| x as u16)
                    .collect::<Vec<u16>>();
                return raw_tag.set_value_to_int16u_vec(int16u_data).map_err(|e| {
                    io_error_plain!(Other, format!("Could not decode tag {:?}: {}", raw_tag, e))
                });
            }
            ...
            (ExifTagFormat::INT8U, ExifTagFormat::INT16U) => {
                ...
                return raw_tag.set_value_to_int8u_vec(int8u_data).map_err(|e| {
                    io_error_plain!(Other, format!("Could not decode tag {:?}: {}", raw_tag, e))
                });
            }

            (ExifTagFormat::INT8U, ExifTagFormat::STRING) => {
                if raw_tag.as_u16()    == 0x0005            && // GPSAltitudeRef
					raw_tag.get_group() == ExifTagGroup::GPS
                {
                    ...
                    return io_error!(
                        Other,
                        format!(
                            "Unknown tag for combination INT8U vs STRING while decoding: {:?}",
                            raw_tag
                        )
                    );
                }
            ...
            (ExifTagFormat::RATIONAL64S, ExifTagFormat::RATIONAL64U) => {
                ...
                return raw_tag.set_value_to_iR64_vec(iR64_data).map_err(|e| {
                    io_error_plain!(Other, format!("Could not decode tag {:?}: {}", raw_tag, e))
                });
            }
            ...
            (ExifTagFormat::UNDEF, ExifTagFormat::STRING) => {
                if raw_tag.as_u16()    == 0x001b            && // GPSProcessingMethod
					raw_tag.get_group() == ExifTagGroup::GPS
                {
                    return raw_tag.set_value_to_undef(raw_data.clone()).map_err(|e| {
                        io_error_plain!(Other, format!("Could not decode tag {:?}: {}", raw_tag, e))
                    });
                } else {
                    return io_error!(
                        Other,
                        format!(
                            "Unknown tag for combination UNDEF vs STRING while decoding: {:?}",
                            raw_tag
                        )
                    );
                }
        ...
        return ExifTag::from_u16_with_data(hex_tag, &format, raw_data, endian, group).map_err(
