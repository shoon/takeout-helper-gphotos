...
fn check_signature(file_buffer: &Vec<u8>) -> Result<Cursor<&Vec<u8>>, std::io::Error> {
    if file_buffer.len() < 12 {
        return io_error!(
            InvalidData,
            "Can't open WebP file - File too small to contain required signatures!"
        );
    }
...
fn get_next_chunk(cursor: &mut Cursor<&Vec<u8>>) -> Result<RiffChunk, std::io::Error> {
    ...
    bytes_read = cursor.read(&mut chunk_data_buffer)?;
...
pub(crate) fn read_metadata(file_buffer: &Vec<u8>) -> Result<Vec<u8>, std::io::Error> {
    ...
    let (mut cursor, parse_webp_result) = check_exif_in_file(file_buffer)?;
    ...
    loop {
        ...
        let Some(chunk_at_index) = parse_webp_result.get(chunk_index) else {
            return io_error!(
                Other,
                format!(
                    "Could not get chunk descriptor at index {} while traversing WebP file!",
                    chunk_index
                )
            );
        };

        let expected_chunk_type = chunk_at_index.header();
        if chunk_type != expected_chunk_type {
            return io_error!(
                Other,
                format!(
                    "Got unexpected chunk type! Expected {} but got {}",
                    expected_chunk_type, chunk_type
                )
            );
        }
...
fn update_file_size_information(
...
fn convert_to_extended_format(cursor: &mut Cursor<&mut Vec<u8>>) -> Result<(), std::io::Error> {
    // Start by getting the first chunk of the WebP file

    // Clippy is a bit stupid here, the as_ref is required as otherwise the
    // read_cursor provided to get_next_chunk is of a mutable container.
    // This happens again in the function write_metadata further down.
    ...
    let (width, height) = match first_chunk.descriptor().header().as_str() {
        "VP8 " => {
            log::debug!("VP8 !");
            io_error!(Other, "Conversion from Simple File Format with 'VP8' chunk to Extended File Format not yet implemented!")
        }
        "VP8L" => get_dimension_info_from_vp8l_chunk(first_chunk.payload()),
        _ => io_error!(
            Other,
            format!(
                "Expected either 'VP8 ' or 'VP8L' chunk for conversion but got {:?}!",
                first_chunk.descriptor().header().as_str()
            )
        ),
    }?;
    ...
    for byte in width_vec.iter().take(3) {
        vp8x_chunk.push(*byte);
    }
    for byte in height_vec.iter().take(3) {
        vp8x_chunk.push(*byte);
    }
...
fn get_dimension_info_from_vp8l_chunk(payload: &[u8]) -> Result<(u32, u32), std::io::Error> {
    ...
    let width_height_info =
        from_u8_vec_res_macro!(u32, &width_height_info_buffer, &Endian::Little)?;
...
pub(crate) fn clear_metadata(file_buffer: &mut Vec<u8>) -> Result<(), std::io::Error> {
    ...
    let (mut cursor, parse_webp_result) = match check_exif_in_file(file_buffer) {
        Ok((_file, parse_webp_result)) => (Cursor::new(file_buffer), parse_webp_result),
        ...
        match chunk_descriptor_result {
            Ok(chunk_descriptor) => {
                ...
                for pre_exif_chunk in &pre_exif_chunks {
                    chunk_type_found_in_pre_exif_chunks |=
                        pre_exif_chunk.to_lowercase() == chunk_descriptor.header().to_lowercase();
                }
