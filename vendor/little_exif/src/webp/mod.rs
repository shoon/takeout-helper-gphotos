...
fn check_riff_signature(file_buffer: &[u8]) -> Result<(), std::io::Error> {
    let bytes_to_check = match file_buffer.get(0..4) {
        Some(bytes) => bytes,
        None => {
            return io_error!(
                InvalidData,
                "Can't open WebP file - File too small to contain RIFF signature!"
            );
        }
    };
    ...
    if bytes_to_check != RIFF_SIGNATURE {
        return io_error!(
            InvalidData,
            format!(
                "Can't open WebP file - Expected RIFF signature but found {}!",
                from_u8_vec_res_macro!(String, bytes_to_check, &Endian::Big)?
            )
        );
    }
...
fn check_webp_signature(file_buffer: &[u8]) -> Result<(), std::io::Error> {
    let Some(buffer_to_check) = file_buffer.get(8..12) else {
        return io_error!(
            InvalidData,
            "Can't open WebP file - File too small to contain WEBP signature!"
        );
    };
    ...
    if buffer_to_check != WEBP_SIGNATURE {
        return io_error!(
            InvalidData,
            format!(
                "Can't open WebP file - Expected WEBP signature but found {}!",
                from_u8_vec_res_macro!(String, buffer_to_check, &Endian::Big)?
            )
        );
    }
...
fn check_byte_count(file_buffer: &[u8], opt_file: Option<&File>) -> Result<(), std::io::Error> {
    let byte_count = from_u8_vec_res_macro!(u32, &file_buffer[4..8], &Endian::Little)?
        .checked_add(8)
        .ok_or(io_error_plain!(
            InvalidData,
            "Can't open WebP file - Byte count in RIFF header is too large!"
        ))?;
    ...
    if let Some(file) = opt_file {
        if file.metadata()?.len() != byte_count as u64 {
            return io_error!(
                InvalidData,
                "Can't open WebP file - Promised byte count does not correspond with file size!"
            );
        }
...
fn encode_metadata_webp(exif_vec: &[u8]) -> Vec<u8> {
...
pub(crate) fn as_u8_vec(general_encoded_metadata: &[u8]) -> Vec<u8> {
    encode_metadata_webp(general_encoded_metadata)
}

fn get_dimension_info_from_vp8_chunk(payload: &[u8]) -> Result<(u32, u32), std::io::Error> {
