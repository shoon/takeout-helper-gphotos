...
fn check_signature(path: &Path) -> Result<File, std::io::Error> {
    ...
    if bytes_read != 12 {
        return io_error!(
            InvalidData,
            "Can't open WebP file - Can't read & check signature!"
        );
    }
...
fn get_next_chunk<T: Seek + Read>(file: &mut T) -> Result<RiffChunk, std::io::Error> {
...
fn get_next_chunk_descriptor<T: Seek + Read>(
    file: &mut T,
...
pub(crate) fn parse_webp(path: &Path) -> Result<Vec<RiffChunkDescriptor>, std::io::Error> {
...
fn check_exif_in_file(path: &Path) -> Result<(File, Vec<RiffChunkDescriptor>), std::io::Error> {
...
fn update_file_size_information(file: &mut File, delta: i32) -> Result<(), std::io::Error> {
    ...
    if bytes_read != 4 {
        return io_error!(
            Other,
            "Could not read file size information from WebP file!"
        );
    }
...
fn convert_to_extended_format(file: &mut File) -> Result<(), std::io::Error> {
    ...
    let (width, height) = match first_chunk.descriptor().header().as_str() {
        "VP8 " => get_dimension_info_from_vp8_chunk(first_chunk.payload()),
...
fn set_exif_flag(path: &Path, exif_flag_value: bool) -> Result<(), std::io::Error> {
    // Parse the WebP file - if this fails, we surely can't read any metadata
    let parsed_webp_result = parse_webp(path)?;
...
pub(crate) fn clear_metadata(path: &Path) -> Result<(), std::io::Error> {
    // Check the file signature, parse it, check that it has a VP8X chunk and
    // the EXIF flag is set there
...
pub(crate) fn write_metadata(path: &Path, metadata: &Metadata) -> Result<(), std::io::Error> {
...
mod tests {
    ...
    #[test]
    fn clear_metadata() -> Result<(), std::io::Error> {
