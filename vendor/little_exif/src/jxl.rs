...
/// stored JXL images
fn starts_with_iso_bmff_signature(file_buffer: &[u8]) -> bool {
...
fn starts_with_jxl_signature(file_buffer: &[u8]) -> bool {
    file_buffer.starts_with(&JXL_SIGNATURE)
}

fn check_signature(file_buffer: &[u8]) -> Result<(), std::io::Error> {
...
fn check_brob_type_for_exif<T: Seek + Read>(cursor: &mut T) -> Result<bool, std::io::Error> {
...
pub(crate) fn file_read_metadata(path: &Path) -> Result<Vec<u8>, std::io::Error> {
    ...
    if bytes_read != 12 {
        return io_error!(
            InvalidData,
            "Can't open JXL file - Can't read & check ISO BMFF signature!"
        );
    }
...
fn generic_read_metadata<T: Seek + Read>(cursor: &mut T) -> Result<Vec<u8>, std::io::Error> {
    loop {
        ...
        let length = from_u8_vec_res_macro!(u32, &length_buffer, &Endian::Big)?
            .checked_sub(8)
        ...
        match type_buffer {
            ...
            BROB_BOX => {
                ...
                if check_brob_type_for_exif(cursor)? {
                    ...
                    let mut compressed_exif_buffer = vec![0u8; (length - 4) as usize];
...
fn encode_metadata_jxl(exif_vec: &[u8]) -> Vec<u8> {
...
fn find_insert_position(file_buffer: &Vec<u8>) -> Result<usize, std::io::Error> {
    let mut cursor = Cursor::new(file_buffer);
    ...
    loop {
        ...
        match type_buffer {
            IsoBmffBoxType::JXL | IsoBmffBoxType::FTYP => {
