...
fn encode_metadata_jpg(exif_vec: &[u8]) -> Vec<u8> {
...
pub(crate) fn clear_segment(
    ...
    loop {
        ...
        if previous_byte_was_marker_prefix {
            ...
            if byte_buffer[0] == segment_marker
            // Given marker, e.g. for APP1
            {
                ...
                if !do_not_delete_segment_override {
                    ...
                    let cutoff_index = 0 + backup_position as usize + temp_buffer.len();
...
pub(crate) fn file_clear_segment(
    path: &Path,
    segment_marker: u8,
    prefix_bytes: Option<&[u8]>,
) -> Result<(), std::io::Error> {
...
fn skip_ecs<T: Seek + Read>(cursor: &mut T) -> Result<(), std::io::Error> {
    let mut byte_buffer = [0u8; 1]; // A buffer for reading in a byte of data from the file
    let mut previous_byte_was_marker_prefix = false; // A boolean for remembering if the previous byte was a marker prefix (0xFF)
    ...
    loop {
        ...
        if previous_byte_was_marker_prefix {
            match byte_buffer[0] {
                0xd0 | 0xd1 | 0xd2 | 0xd3 | 0xd4 | 0xd5 | 0xd6 | 0xd7 | 0x00 => {
                    // Do nothing
                }
