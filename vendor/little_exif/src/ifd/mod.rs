...
impl ImageFileDirectory {
    pub fn new_with_tags(tags: Vec<ExifTag>, group: ExifTagGroup, nr: u32) -> Self {
        ImageFileDirectory {
            tags: tags,
            ifd_type: group,
    ...
    pub(crate) fn sort_tags(&mut self) {
        self.tags.sort_by_key(|tag| tag.as_u16());
    }
    ...
    pub(crate) fn decode_ifd(
        ...
        if required > available {
            return io_error!(
                Other,
                format!(
                    "Not enough data to decode IFD! Required: {} Available: {}",
                    required, available
                )
            );
        }
        ...
        for _ in 0..number_of_entries {
            ...
            let Some(byte_count) = format
                .bytes_per_component()
                .checked_mul(hex_component_number)
            else {
                return io_error!(
                    Other,
                    format!("Byte count overflow for tag 0x{:04x}!", hex_tag)
                );
            };
            ...
            if tag_result.is_err() {
                ...
                match ExifTag::from_u16_with_data(hex_tag, &format, &raw_data, endian, group) {
                    ...
                    Err(e) => {
                        return io_error!(
                            Other,
                            format!("Could not construct unknown tag 0x{:04x}: {}", hex_tag, e)
                        )
                    }
                }
            ...
            if let TagType::IFD_OFFSET(subifd_group) = tag.get_tag_type() {
                ...
                if let Ok(_subifd_result) = subifd_decode_result {
                    ...
                    return io_error!(
                        Other,
                        format!("Could not decode SubIFD {subifd_group:?}:\n {decode_err:?}")
                    );
                }
            }
        ...
        if let (Some(strip_tags_0), Some(strip_tags_1)) = strip_tags {
            // 0 -> offsets
            // 1 -> byte counts
            if let (TagType::DATA_OFFSET(offsets), TagType::DATA_OFFSET(byte_counts)) =
                (strip_tags_0.get_tag_type(), strip_tags_1.get_tag_type())
            {
                let backup_position = data_cursor.position();
        ...
        if let (Some(thumbnail_info_0), Some(thumbnail_info_1)) = thumbnail_info {
            // 0 -> offset
            // 1 -> length
            if let (TagType::DATA_OFFSET(offset), TagType::DATA_OFFSET(length)) = (
                ...
                if offset.len() == 1 && length.len() == 1 {
                    ...
                    log::warn!("Can't decode thumbnail! The ThumbnailOffset and ThumbnailLength tags are expected to contain exactly 1 INT32U value. However, they have {} and {} values.", offset.len(), length.len());
                }
        ...
        let link_is_zero = next_ifd_link_buffer
            .iter()
        ...
        return Ok(Some(from_u8_vec_res_macro!(
            u32,
            &next_ifd_link_buffer,
            endian
        )?));
    }
    ...
    pub(crate) fn encode_ifd(
        ...
        if let Some(ifd_with_offset_info_only) = ifds_with_offset_info_only.iter().find(|ifd| {
        ...
        let count_entries = all_relevant_tags
            .iter()
            .filter(|tag| {
                tag.is_writable()
                    || matches!(
                        tag.get_tag_type(),
                        TagType::IFD_OFFSET(_) | TagType::DATA_OFFSET(_)
                    )
            })
            .count() as u16;
        ...
        *current_offset += alignment_count;
        ...
        for tag in &all_relevant_tags {
            ...
            let value = match tag.get_tag_type() {
                TagType::VALUE => tag.value_as_u8_vec(&data.get_endian()),
                ...
                TagType::IFD_OFFSET(_) => {
                    if let Some(group) = Self::get_ifd_type_for_offset_tag(tag) {
                        ...
                        if let Some(found_ifd) = data.get_ifds().iter().find(|ifd| {
                            ifd.get_generic_ifd_nr() == self.get_generic_ifd_nr()
                            ...
                            return io_error!(
                                Other,
                                format!(
                                    "Could not find SubIFD {group:?} for offset tag 0x{:04x}!",
                                    tag.as_u16()
                                )
                            );
                        }
                        ...
                        return io_error!(
                            Other,
                            format!(
                                "Could not determine SubIFD type for offset tag 0x{:04x}!",
                                tag.as_u16()
                            )
                        );
                    }
            ...
            *current_offset += alignment_count;
            ...
            if tag.is_string() {
                string_padding = vec![0x00; number_of_components as usize - value.len()];
            }
        ...
        return Ok((
            (ifd_offset + 2 + IFD_ENTRY_LENGTH * count_entries as u32) as u64,
            ifd_offset_vec,
        ));
    }
}
