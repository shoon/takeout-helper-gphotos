...
pub(crate) fn generic_write_metadata<T: Seek + Write>(
    cursor: &mut T,
    metadata: &Metadata,
) -> Result<(), std::io::Error> {
...
fn check_for_required_tags(metadata: &Metadata) -> Result<(), std::io::Error> {
    ...
    if metadata.get_tag_by_hex(0x0106, Some(GENERIC)).count() == 0 {
        return io_error!(
            NotFound,
            "TIFF requires PhotometricInterpretation (0x0106) tag!"
        );
    }
    ...
    if metadata.get_tag_by_hex(0x0102, Some(GENERIC)).count() == 0 {
        log::warn!(
            "All TIFF variants (except for bilevel graphics) require BitsPerSample (0x0102) tag!"
        );
    }
