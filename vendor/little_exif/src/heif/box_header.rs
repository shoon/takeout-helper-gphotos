...
impl BoxHeader {
    ...
    pub(crate) fn new_exif_info_entry_box_header() -> Self {
    ...
    pub(crate) fn new_simple_box_header() -> Self {
        Self {
            box_size: 8,
            largesize: false,
            box_type: BoxType::unknown {
                box_type: "tobi".to_owned(),
            },
            header_size: 8,
            version: None,
            flags: None,
    ...
    pub(crate) fn new_full_box_header() -> Self {
        Self {
            box_size: 12,
            largesize: false,
            box_type: BoxType::unknown {
    ...
    pub(super) fn read_box_header<T: Seek + Read>(cursor: &mut T) -> Result<Self, std::io::Error> {
        // Read in the size
    ...
    pub(super) fn get_box_size(&self) -> u64 {
        return self.box_size;
    }

    pub(super) fn get_box_type(&self) -> BoxType {
        return self.box_type.clone();
    }

    pub(super) fn get_header_size(&self) -> u64 {
        return self.header_size;
    }

    pub(super) fn set_box_size(&mut self, new_size: u64) {
        self.box_size = new_size;
    }

    pub(super) fn set_box_type_via_string(&mut self, new_type: &str) {
        self.box_type = BoxType::from_4_bytes(match new_type.as_bytes().try_into() {
            Ok(arr) => arr,
    ...
    pub(super) fn set_version(&mut self, new_version: Option<u8>) {
    ...
    pub(super) fn serialize(&self) -> Vec<u8> {
        ...
        if self.box_type.extends_fullbox() {
            serialized.push(
                self.version
                    .expect("BoxHeader: version is not set when serializing"),
            );
            for flag in self
                .flags
                .expect("BoxHeader: flags not set when serializing")
            {
