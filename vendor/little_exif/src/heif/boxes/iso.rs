...
impl IsoBox {
    fn construct_from_cursor_unboxed<T: Seek + Read>(
        ...
        let Some(data_left_to_read) = header.get_box_size().checked_sub(header.get_header_size())
        else {
