...
impl MetaBox {
    pub(crate) fn get_item_info_box(&self) -> Result<&ItemInfoBox, std::io::Error> {
        match self
            .other_boxes
            .iter()
            .find(|b| b.get_header().get_box_type() == BoxType::iinf)
        {
            ...
            None => io_error!(NotFound, "No iinf box found in MetaBox"),
        }
    }

    pub(crate) fn get_item_location_box(&self) -> Result<&ItemLocationBox, std::io::Error> {
        match self
            .other_boxes
            .iter()
            .find(|b| b.get_header().get_box_type() == BoxType::iloc)
        {
            ...
            None => io_error!(NotFound, "No iloc box found in MetaBox"),
        }
    }

    pub(crate) fn get_item_location_box_mut(
        &mut self,
    ...
    pub(crate) fn get_item_reference_box(&self) -> Option<&ItemReferenceBox> {
        if let Some(found_box) = self
            .other_boxes
            .iter()
    ...
    pub(crate) fn create_new_item_reference_box_if_none_exists_yet(&mut self) -> u64 {
...
impl ParsableIsoBox for MetaBox {
    fn construct_from_cursor<T: Seek + Read>(
        ...
        cursor
            ...
            .read_to_end(&mut meta_box_bytes)?;
        ...
        while local_cursor.position() < remaining_bytes as u64 {
            ...
            let sub_box = read_box_based_on_header(&mut local_cursor, sub_header)?;
