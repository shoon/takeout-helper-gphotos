...
pub(crate) enum ItemConstructionMethod {
    FILE = 0,
    IDAT = 1,
    ITEM = 2,
}
...
#[allow(dead_code)]
#[derive(Debug)]
pub(crate) struct ItemLocationEntry {
...
#[allow(dead_code)]
#[derive(Debug)]
pub(crate) struct ItemLocationEntryExtentEntry {
...
impl ItemLocationEntryExtentEntry {
    fn read_from_cursor<T: Seek + Read>(
        cursor: &mut T,
        header: &BoxHeader,
        offset_size: u8,
        length_size: u8,
...
impl ItemLocationEntry {
    fn read_from_cursor<T: Seek + Read>(
        ...
        let item_id = match header.get_version() {
            ...
            _ => {
                return io_error!(
                    Other,
                    "Invalid version for ItemLocationEntry decode!".to_string()
                )
            }
        };
    ...
    pub(crate) fn get_construction_method(&self) -> ItemConstructionMethod {
...
impl ItemLocationBox {
    fn construct_from_cursor_unboxed<T: Seek + Read>(
        ...
        let item_count = match header.get_version() {
            ...
            _ => {
                return io_error!(
                    Other,
                    "Invalid version for ItemLocationBox decode!".to_string()
                )
            }
        };
    ...
    pub(crate) fn get_item_location_entry(
        &self,
        item_id: u16,
    ...
    pub(crate) fn create_new_item_location_entry(
        ...
        return (
            self.items
                .last_mut()
                .expect("No items present after insertion")
                .item_id,
...
impl ParsableIsoBox for ItemLocationBox {
            ...
            for extent in &item.extents {
                    ...
                    && self.index_size > 0
                {
                    match self.index_size {
                        4 => {
                            let idx: u32 =
                                extent.extent_index.expect("Extent index missing") as u32;
