...
pub trait U8conversion<T> {
    fn to_u8_vec(&self, endian: &Endian) -> Vec<u8>;

    fn from_u8_vec(u8_vec: &[u8], endian: &Endian) -> T {
    ...
    fn from_u8_vec_res(u8_vec: &[u8], endian: &Endian) -> Result<T, io::Error>;
}
...
macro_rules! build_u8conversion {
    (
        ...
        impl U8conversion<$type> for $type {
            ...
            fn from_u8_vec_res(u8_vec: &[u8], endian: &Endian) -> Result<$type, io::Error> {
                ...
                let res = match *endian {
                    Endian::Little => <paste! {[<$type>]}>::from_le_bytes(
                        u8_vec[0..$number_of_bytes].try_into().map_err(|_| {
                            io::Error::new(
                                io::ErrorKind::InvalidData,
                                "from_u8_vec_res: Mangled EXIF data encountered!",
                            )
...
impl U8conversion<String> for String {
    fn to_u8_vec(&self, _endian: &Endian) -> Vec<u8> {
        let mut u8_vec = self.as_bytes().to_vec();
        u8_vec.push(0x00_u8);
        return u8_vec;
    }

    fn from_u8_vec_res(u8_vec: &[u8], _endian: &Endian) -> Result<String, io::Error> {
...
impl U8conversion<uR64> for uR64 {
    ...
    fn from_u8_vec_res(u8_vec: &[u8], endian: &Endian) -> Result<uR64, io::Error> {
        if u8_vec.len() != 8 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "from_u8_vec_res: Mangled EXIF data encountered!",
...
impl U8conversion<iR64> for iR64 {
    ...
    fn from_u8_vec_res(u8_vec: &[u8], endian: &Endian) -> Result<iR64, io::Error> {
        if u8_vec.len() != 8 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "from_u8_vec_res: Mangled EXIF data encountered!",
...
macro_rules! build_vec_u8conversion {
    (
        ...
        impl U8conversion<Vec<$type>> for Vec<$type> {
            ...
            fn from_u8_vec_res(u8_vec: &[u8], endian: &Endian) -> Result<Vec<$type>, io::Error> {
                ...
                if u8_vec.len() % $number_of_bytes != 0 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "from_u8_vec_res (Vec): Mangled EXIF data encountered!",
                    ));
                }
